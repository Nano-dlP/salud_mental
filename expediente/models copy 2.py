...
from core.models import Rol, Sede
...
class Expediente(models.Model):
    ...
    expediente_fisico = models.BooleanField('Expediente Físico', default=False)
    ...
    edad_persona = models.PositiveIntegerField('Edad Persona', null=True, blank=True)
    ...
    class Meta:
        unique_together = ('numero', 'anio', 'sede')
    ...
    def save(self, *args, **kwargs):
        if not self.pk:  # Si es un nuevo expediente
            ...
            # Buscamos el último expediente de la misma sede y año
            ...
            self.identificador = f"{self.sede.abreviatura.upper()}-{str(self.numero).zfill(5)}-{self.anio}"
    ...
    def save(self, *args, **kwargs):
        if not self.pk: #Esto comprueba si el expediente es nuevo (es decir, no tiene clave primaria aún, no fue guardado). Solo ejecuta la lógica si es un nuevo expediente.
            ...
            self.anio = today.year
            #Busca el último expediente creado en esa sede y en ese año, ordenado de mayor a menor por numero. Si existe un expediente anterior, se lo guarda en ultimo. Si no, ultimo será None.
            ultimo = Expediente.objects.filter(
                sede=self.sede,
                anio=self.anio
            ).order_by('-numero').first()
            ...
            #Si no hay ninguno anterior (es el primero del año en esa sede), el número será 1.
            if ultimo:
